...
- **Static assets** go in `astro/public/` (self-hosted slide PDFs in `astro/public/slides/`, talk images, avatars, `CNAME`). The CV PDF is generated, not stored here.
...
- **CV PDF**: resume data lives in `astro/src/data/resume.ts`, rendered by both `/resume/` and `astro/src/pages/cv.json.ts` (emits `dist/cv.json`). `npm run build:pdf` runs `typst compile` on `astro/cv/cv.typ`, which reads that JSON and writes `dist/CV_Bauke_Brenninkmeijer.pdf` (two-column, Inter from `astro/cv/fonts/`). The template asserts a two-page budget, so an overlong CV fails the compile. Needs the `typst` binary (`brew install typst`; CI installs a pinned release).
