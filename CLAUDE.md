...
- **CV PDF**: `astro/public/` holds no CV. `npm run build:pdf` (`astro/scripts/build-pdf.mjs`) prints the built `/resume/` page to `dist/CV_Bauke_Brenninkmeijer.pdf` with headless Chrome, after `astro build`. The resume page is the only source; print-only blocks in `resume.astro` (contact header, summary, skills, languages) are `hidden print:block`, and the `@media print` rules live at the bottom of `global.css`.
