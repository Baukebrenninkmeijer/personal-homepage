...
| **Recall at low k** | Pull the gold chunk into a short window when base retrieval misses it | Partially -- tested with a legacy `ms-marco-MiniLM-L-6-v2` cross-encoder only | Open -- modern rerankers (Cohere, Voyage, `bge-reranker-v2`) not evaluated |
...
- **Single legacy reranker**: `cross-encoder/ms-marco-MiniLM-L-6-v2` is a 2019-era, 22M-parameter generic reranker. Modern options (Cohere Rerank, Voyage, `bge-reranker-v2-m3`) are untested here.
